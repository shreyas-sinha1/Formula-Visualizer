...
function axis() {
  return (
    <>
      <line x1="40" x2="600" y1="180" y2="180" stroke="#c8d2dd" strokeWidth="1.5" />
      <line x1="320" x2="320" y1="34" y2="326" stroke="#c8d2dd" strokeWidth="1.5" />
      {[-3, -2, -1, 1, 2, 3].map((tick) => (
        <g key={tick}>
          <line x1={mapX(tick)} x2={mapX(tick)} y1="175" y2="185" stroke="#c8d2dd" />
          <line x1="315" x2="325" y1={mapY(tick)} y2={mapY(tick)} stroke="#c8d2dd" />
        </g>
      ))}
    </>
  );
...
function WaveVisual({ values, type }) {
  ...
  return (
    <GraphFrame>
      ...
      <text x="48" y="40" fill="#475569" fontSize="14">
...
function EulerVisual({ values }) {
  ...
  return (
    <GraphFrame>
      ...
      <text x={mapX(0) + 10} y={mapY(scale) - 10} fill="#475569" fontSize="13">
        natural base e
      </text>
    </GraphFrame>
  );
...
function DerivativeVisual({ values }) {
  ...
  return (
    <GraphFrame>
      ...
      <circle cx={mapX(x)} cy={mapY(yAtX)} r="7" fill="#f59e0b" stroke="white" strokeWidth="3" />
      <text x="48" y="40" fill="#475569" fontSize="14">
...
function GraphFrame({ children }) {
  return (
    <svg viewBox="0 0 640 360" className="h-full min-h-[340px] w-full rounded-lg border border-slate-200 bg-white">
...
function Summary({ lines }) {
  return (
    <div className="rounded-lg border border-slate-200 bg-slate-50 p-4">
      <p className="mb-3 text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">Live values</p>
      <div className="space-y-3">
        {lines.map((line) => (
          <p key={line} className="rounded-md bg-white px-3 py-2 font-mono text-sm text-slate-700 shadow-sm">
            {line}
          </p>
        ))}
      </div>
  ...
  return (
    <main className="min-h-screen px-4 py-6 text-slate-900 sm:px-6 lg:px-8">
      <div className="mx-auto grid max-w-7xl gap-6 lg:grid-cols-[300px_1fr]">
        <aside className="rounded-lg border border-slate-200 bg-white p-4 shadow-soft lg:sticky lg:top-6 lg:h-[calc(100vh-48px)]">
          <div className="mb-5">
            <p className="text-sm font-medium text-teal-700">Formula Visualizer</p>
            <h1 className="mt-2 text-2xl font-semibold tracking-normal text-slate-950">Interactive math views</h1>
          </div>
            ...
            {formulas.map((formula) => {
              ...
              return (
                ...
                >
                  ...
                  <span className="mt-1 block truncate font-mono text-xs text-slate-500">{formula.expression}</span>
                </button>
              );
            })}
          </div>
        ...
        <section className="space-y-6">
          <div className="rounded-lg border border-slate-200 bg-white p-5 shadow-soft">
            <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
              <div>
                <p className="text-sm font-medium text-slate-500">Formula {formulaIndex} of {formulas.length}</p>
                <h2 className="mt-1 text-3xl font-semibold tracking-normal text-slate-950">{selectedFormula.name}</h2>
                <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-600">{selectedFormula.description}</p>
              </div>
          ...
          <div className="grid gap-6 xl:grid-cols-[1fr_320px]">
            ...
            <div className="rounded-lg border border-slate-200 bg-white p-4 shadow-soft">
              <div className="mb-5">
                <h3 className="text-lg font-semibold text-slate-950">Controls</h3>
                <p className="mt-1 text-sm text-slate-500">Slide values and watch the visual update.</p>
              </div>
